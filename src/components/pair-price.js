...
class PairPrice extends Component {
    shouldComponentUpdate(nextProps){
      const answer = this.props && this.props.pair && (this.props.price !== nextProps.price || this.props.pair.key !== nextProps.pair.key || this.props.pair.status !== nextProps.pair.status);
      return answer;
    }
            ...
            onClick={ () =>
              this.props.selectPriceClick(numeral(
                this.props.pair
                  ...
                  : 0)
                  .format('0.[00000000]')) }>
                  Last trade was <span className={ this.props.pair && this.props.pair.status ? this.props.pair.status : ""}>
                    { numeral(
              this.props.pair
                ? this.props.pair.last
                : 0)
                .format((parseInt(this.props.pair && this.props.pair.last, 10) >= 1) ? '0.[00000000]' : '0.00000000') }
              </span>
            </span>
          </div>
        </div>
